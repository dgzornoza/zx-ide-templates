...
void sp1_clear_screen_tiles(void) __z88dk_fastcall
{
    ...
    zx_border(DEFAULT_SCREEN_PAPER_COLOR);

    const struct sp1_Rect game_area = {0, 0, SYS_SCREEN_CHARS_WIDTH, SYS_SCREEN_CHARS_HEIGHT};
    sp1_ClearRectInv(&game_area, DEFAULT_SCREEN_INK_COLOR | DEFAULT_SCREEN_PAPER_COLOR, (uint16_t)' ', SP1_RFLAG_TILE | SP1_RFLAG_COLOUR | SP1_RFLAG_SPRITE);
}
