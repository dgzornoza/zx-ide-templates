...
{
    /* Sets up draw_map_config from the
     * HUD asset headers and dispatches draw_map_static_region() + draw_map_static_region_mirrored().
     *
     * Remarks: The HUD is static and doesn’t need to be redrawn in the same scene,
     * so its surface won’t be invalidated for repainting.
     * This means you can use just a temporary tile to draw the whole HUD statically,
     * freeing up more tile slots for the dynamic background.
     */
